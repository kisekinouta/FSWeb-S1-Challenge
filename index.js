...
function fenomenSil(fenoDizi, index) {
  let newfenoDizi = [...fenoDizi];
  newfenoDizi.splice(index, 1);
  return newfenoDizi;
}
...
function fenomenEkle(feno, number, name, follower, post, platform) {
  feno = [...fenomenler];
  feno.push({
    "number": 6,
    "profile": "Workintech",
...
function enFenomenler(fenomenlerArray) {
  let newArray = []; 
  for (let i = 0; i < fenomenlerArray.length; i++) {
  if(fenomenlerArray[i].followers > 100000000) {
    newArray.push(fenomenlerArray[i].profile);
  }
  }
  return newArray;
}
...
function fenomenGonderimSayisi(fenomenlerArray, objectProfile) {
  for(let object of fenomenlerArray) {
...
function platformaGoreCokGonderiYapanFenomen(anArray, aPlatform) {
  let postsArray = []
  for(let object of anArray) {
    if(object.platform === aPlatform){
      postsArray.push(object.posts)
    }
  }
let maximumPost = Math.max(...postsArray)
for(let object of anArray) {
